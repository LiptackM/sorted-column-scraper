"""
unit tests for column_sorter.py functions

notes:
    - requires a test_file & test_corrupt_file (see fixtures) in the same
      folder as this test.  Note if you provide these, also change expected
      values below.
    - test_corrupt-file is just a simple text file renamed to a xlsx so it is
      corrupt.


to run with coverage (coverage suggested, initally this was 87%, when
        only skipping simple I/O in main()):
    > pytest -rs --cov=sorted_column_scraper --cov-report=term-missing
...
from sorted_column_scraper.column_sorter import process_excel_columns_rw
from sorted_column_scraper.column_sorter import remove_before_and_after
from sorted_column_scraper.column_sorter import process_and_sort
from sorted_column_scraper.column_sorter import print_output
from sorted_column_scraper.column_sorter import parse_args


@pytest.fixture
def test_file():
    return r"tests\excel_column_sorted_test_content.xlsx"


@pytest.fixture
def test_corrupt_file():
    return r"tests\excel_column_sorted_test_corrupt.xlsx"

# ___process_excel_columns_rw _____________________________________
...
def test_process_excel_columns_rw_corrupted_xls(test_corrupt_file):
    with pytest.raises(zipfile.BadZipFile, match="File is not a zip file"):
        # should raise the exception directly for this generator
