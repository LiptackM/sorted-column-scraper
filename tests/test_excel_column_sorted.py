...
def test_process_excel_columns_rw_no_such_file():    
    with pytest.raises(FileNotFoundError, match="No such file or directory"):
        list(process_excel_columns_rw('excel_column_sorted_test_nonexistent.xlsx', 'Test sheet'))  
...
def test_remove_before_and_after_positive():
    ...
    assert remove_before_and_after(my_list, None, None) == ["TRUE", "FALSE", "", "text", "my Text", 0.1, -10, '#Div/0!']
...
def test_parse_args_correct(test_file):
    argv = ["excel_column_sorted.py", test_file, "Test sheet", "Column A", "None"]
...
def test_parse_args_too_many_arguments(test_file):
    argv = ["excel_column_sorted.py", test_file, "Test sheet", "Column A", "None", "ExtraArg"]
    result = parse_args(argv)
    expected = (Path(test_file), "Test sheet", "Column A", "None")
    assert result == expected
